...
int compararPuntosEnX(const void *p1, const void *p2) {
...
int main(int argc, char *argv[]) {
    ...
    cPuntos= new Punto[n];
    for (int i= 0; i<n; i++) {
        cPuntos[i].x= 10000.0*(rand()/(1.0*RAND_MAX));
        cPuntos[i].y= 10000.0*(rand()/(1.0*RAND_MAX));
    }

    // Ordenamos los puntos por componente x
    qsort(cPuntos, n, sizeof(Punto), compararPuntosEnX);
